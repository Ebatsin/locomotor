...
public class Gaussian {
	...
	private Gaussian() {
		...
		for(int i = 0; i <= _steps / 2; ++i) {
			...
			value = Math.exp(-x * x / 2) / sqrt2PI;
			_pdfLookUp.put(x, value);
			_pdfLookUp.put(-x, value);
		}
		...
		for(int i = 0; i <= _steps / 2; ++i) {
			...
			_cdfLookUp.put(x, value);
			_cdfLookUp.put(-x, 1 - value);
		}
	}

	private double getNearestStep(double x) {
		return Math.round(x / _stepsLength) * _stepsLength;
	}
	...
	* @param x pdf(x)
	*/
	public double truePdf(double x) {
	...
	* @param x cdf(x)
	*/
	public double trueCdf(double x) {
		if(x < -4) {
			return 0;
		}
		if(x > 4) {
	...
	*/
	public double pdf(double x) {
		...
		return _pdfLookUp.get(getNearestStep(x));
	}
	...
	*/
	public double cdf(double x) {
		...
		return _cdfLookUp.get(getNearestStep(x));
	}
}